...
void BlueMagicCameraController::focus(float focus)
{
  ...
  _cameraControl->writeValue(data, 10, true);
  // 写入 Focus 信息到 BlueMagicState 对象 _state
...
void BlueMagicCameraController::zoomInt(int32_t zoom)
{
  if (zoom < 2 && zoom > 300)
    return;

  uint8_t xlow = zoom & 0xff;
  uint8_t xhigh = (zoom >> 8);

  uint8_t data[10] = {255, 6, 0, 0, 0, 7, 128, 0, xlow, xhigh};
  _cameraControl->writeValue(data, 10, true);
}
